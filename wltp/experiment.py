...
def calcEngineRevs_required(V, gear_ratios, n_idle, v_stopped_threshold):
    ...
    _GEAR_RATIOS = np.tile(
        gear_ratios, (nV, 1)
    ).T  ## TODO: Prepend row for idle-gear in _N_GEARS
    _N_GEARS = np.tile(V, (nG, 1)) * _GEAR_RATIOS
    assert _GEARS.shape == _GEAR_RATIOS.shape == _N_GEARS.shape, _shapes(
        _GEARS, _GEAR_RATIOS, _N_GEARS, V
    )
...
def possibleGears_byEngineRevs(
    V,
    A,
    _N_GEARS,
    ngears,
    ...
    addDriveabilityProblems(
        _GEARS_BAD, "g%i: Revolutions too high!" % ngears, driveability_issues
    )
    ...
    GEARS_YES_MIN[0, :] = (_N_GEARS[0, :] >= n_idle) | (
        V <= v_stopped_threshold
    ...
    N_GEARS2 = _N_GEARS[1, :]
    GEARS_YES_MIN[1, :] = ((N_GEARS2 >= n_min_gear2) & (A <= 0)) | (
        (N_GEARS2 >= n_min_gear2) & (A > 0)
    )

    ## Revert impossibles to min-gear, n_min & clutched.
    #
    _GEARS_BAD = CLUTCH = (~GEARS_YES_MIN).all(
        axis=0
    )  ## TODO: Clutch on gear2 (f=will be fixed when add also CLUTCH_GEARS)
...
def calcPower_available(
    _N_GEARS, n_idle, n_rated, p_rated, load_curve, p_safety_margin
):
    """
    ...
    _P_WOTS = np.interp(
        _N_NORMS, load_curve["n_norm"], load_curve["p_norm"]
    )  # When outside of load_curve, accept max-min gear.
...
def possibleGears_byPower(
    ...
    (_P_AVAILS, _N_NORMS) = calcPower_available(
        _N_GEARS, n_idle, n_rated, p_rated, load_curve, p_safety_margin
    )
...
def selectGears(_GEARS, _G_BY_N, _G_BY_P, driveability_issues):
    ...
    _GEARS[
        ~_GEARS_YES
    ] = -1  ## NOTE: Invalid gears will be smoothed-away in extra_rule(1).
    assert _G_BY_N.dtype == _G_BY_P.dtype == "bool", _dtypes(_G_BY_N, _G_BY_P)
